...
if %errorlevel% neq 0 (
  ...
  echo.
  powershell -NoProfile -Command "Start-Process netsh -ArgumentList 'advfirewall','firewall','add','rule','name=EasyTransfert','dir=in','action=allow','protocol=TCP','localport=4455','profile=private' -Verb RunAs -Wait" >nul 2>&1
)
